...
import be.he2b.atlr5.skyjo.db.SequenceDB;
...
import be.he2b.atlr5.skyjo.exception.RepositoryException;
...
public class AdminModel implements AdminFacade {
    ...
    public void insertPause(int id_user, int id_game) throws DtoException{
        
        try {
            DBManager.startTransaction();
            PauseBusiness.inserPause(id_user, id_game);
            DBManager.validateTransaction();
        } catch (DBException ex) {
            Logger.getLogger(AdminModel.class.getName()).log(Level.SEVERE, null, ex);
        }
     ...
     * @throws DBBusinessException the db business exception
     */
    public void addDataFromGame(List<Integer> usersId, List<Integer> usersScore, int srvID) throws DBBusinessException, RepositoryException {
        try {
            DBManager.startTransaction();
            for (int i = 0; i < usersId.size(); i++) {
                try {
