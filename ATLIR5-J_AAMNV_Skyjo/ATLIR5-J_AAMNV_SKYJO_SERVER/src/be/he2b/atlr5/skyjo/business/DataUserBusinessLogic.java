...
import be.he2b.atlr5.skyjo.db.DataUserDB;
...
public class DataUserBusinessLogic {
    ...
    static void updateUserStat(int idUser,int score) throws DBException{
        ...
        switch (col.size()) {
            case 1 -> {
                ...
                var Datadb = new DataUserDB();
                Datadb.update(a);
            }
            case 0 -> {
                try {
                    //DataUserDB.insert(new DataUserDto(idUser, 1, score, 0));
                    var datadb = new DataUserDB();
                    datadb.insert(new DataUserDto(idUser,1, score, 0));
    ...
    static int add(int idUser) throws DBException {
        try {
            //return DataUserDB.insert(new DataUserDto(idUser,0,99,0));
            var datadb = new DataUserDB();
            datadb.insert(new DataUserDto(idUser, 0, 99, 0));
        } catch (DtoException ex) {
